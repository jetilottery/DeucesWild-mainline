define(/**
    ...
    require => {
        ...
        function init() {
            ...
            displayList.infoBarText = new FittedText(displayList.infoBarText.text);
        ...
        function revealAll() {
            let rows = [
                pickPointArray.slice(0, 5).filter(e => !e.revealed).map((e, i) => Tween.delayedCall(gameConfig.autoPlayPlayerNumberInterval * i + gameConfig.autoPlayPlayerRowInterval, e.reveal)),
                pickPointArray.slice(5, 10).filter(e => !e.revealed).map((e, i) => Tween.delayedCall(gameConfig.autoPlayPlayerNumberInterval * (i + 4) + gameConfig.autoPlayPlayerRowInterval * 2, e.reveal)),
                pickPointArray.slice(10, 15).filter(e => !e.revealed).map((e, i) => Tween.delayedCall(gameConfig.autoPlayPlayerNumberInterval * (i + 8)   + gameConfig.autoPlayPlayerRowInterval * 3, e.reveal)),
                pickPointArray.slice(15, 20).filter(e => !e.revealed).map((e, i) => Tween.delayedCall(gameConfig.autoPlayPlayerNumberInterval * (i + 12)  + gameConfig.autoPlayPlayerRowInterval * 4, e.reveal)),
            ];
